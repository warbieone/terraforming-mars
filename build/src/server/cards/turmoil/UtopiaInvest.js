...
class UtopiaInvest extends CorporationCard_1.CorporationCard {
    constructor() {
        super({
    ...
    action(player) {
        ...
        const options = [];
        const reduceMegacredits = new SelectOption_1.SelectOption('Decrease M€ production', 'Decrease production').andThen(() => {
            player.production.add(Resource_1.Resource.MEGACREDITS, -1);
            player.megaCredits += 4;
            this.log(player, 'megacredit');
            return undefined;
        });
        const reduceSteel = new SelectOption_1.SelectOption('Decrease steel production', 'Decrease production').andThen(() => {
            player.production.add(Resource_1.Resource.STEEL, -1);
            player.steel += 4;
            this.log(player, 'steel');
            return undefined;
        });
        const reduceTitanium = new SelectOption_1.SelectOption('Decrease titanium production', 'Decrease production').andThen(() => {
            player.production.add(Resource_1.Resource.TITANIUM, -1);
            player.titanium += 4;
            this.log(player, 'titanium');
            return undefined;
        });
        const reducePlants = new SelectOption_1.SelectOption('Decrease plants production', 'Decrease production').andThen(() => {
            player.production.add(Resource_1.Resource.PLANTS, -1);
            player.plants += 4;
            this.log(player, 'plant');
            return undefined;
        });
        const reduceEnergy = new SelectOption_1.SelectOption('Decrease energy production', 'Decrease production').andThen(() => {
            player.production.add(Resource_1.Resource.ENERGY, -1);
            player.energy += 4;
            this.log(player, 'energy');
            return undefined;
        });
        const reduceHeat = new SelectOption_1.SelectOption('Decrease heat production', 'Decrease production').andThen(() => {
