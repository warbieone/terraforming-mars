...
class MarketManipulation extends Card_1.Card {
    ...
    bespokePlay(player) {
        ...
        const increaseColonyTrack = new SelectColony_1.SelectColony('Select which colony tile track to increase', 'Increase', increasableColonies)
            .andThen((increasedColony) => {
            increasedColony.increaseTrack();
            LogHelper_1.LogHelper.logColonyTrackIncrease(player, increasedColony, 1);
            const decreaseColonyTrack = new SelectColony_1.SelectColony('Select which colony tile track to decrease', 'Decrease', decreasableColonies.filter((decreaseableColony) => decreaseableColony.name !== increasedColony.name))
