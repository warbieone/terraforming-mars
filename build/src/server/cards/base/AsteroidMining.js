"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AsteroidMining = void 0;
const Tag_1 = require("../../../common/cards/Tag");
const Card_1 = require("../Card");
const CardType_1 = require("../../../common/cards/CardType");
const CardName_1 = require("../../../common/cards/CardName");
const CardRenderer_1 = require("../render/CardRenderer");
class AsteroidMining extends Card_1.Card {
    constructor() {
        super({
            type: CardType_1.CardType.AUTOMATED,
            name: CardName_1.CardName.ASTEROID_MINING,
            tags: [Tag_1.Tag.JOVIAN, Tag_1.Tag.SPACE],
            cost: 30,
            victoryPoints: 2,
            behavior: {
                production: { titanium: 2 },
            },
            metadata: {
                description: 'Increase your titanium production 2 steps.',
                cardNumber: '040',
                renderData: CardRenderer_1.CardRenderer.builder((b) => b.production((pb) => pb.titanium(2))),
            },
        });
    }
}
exports.AsteroidMining = AsteroidMining;
