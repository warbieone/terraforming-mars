    ...
    resolve(game, turmoil) {
        game.getPlayersInGenerationOrder().forEach((player) => {
            if (player.cardsInHand.length >= 2) {
                game.defer(new DiscardCards_1.DiscardCards(player, 2, 2, 'Global Event - Select 2 cards to discard'));
            }
            else if (player.cardsInHand.length === 1) {
                game.defer(new DiscardCards_1.DiscardCards(player, 1, 1, 'Global Event - Select a card to discard'));
            }
            player.stock.add(Resource_1.Resource.MEGACREDITS, 2 * (turmoil.getPlayerInfluence(player)), { log: true, from: this.name });
        });
    }
}
