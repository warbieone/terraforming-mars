...
const RENDER_DATA = CardRenderer_1.CardRenderer.builder((b) => {
    b.megacredits(2).slash().tag(Tag_1.Tag.EARTH).influence({ size: Size_1.Size.SMALL });
});
class HomeworldSupport extends GlobalEvent_1.GlobalEvent {
    ...
    resolve(game, turmoil) {
        game.getPlayersInGenerationOrder().forEach((player) => {
            ...
            if (amount > 0) {
                player.stock.add(Resource_1.Resource.MEGACREDITS, 2 * amount, { log: true, from: this.name });
            }
        });
    }
}
