...
const RENDER_DATA = CardRenderer_1.CardRenderer.builder((b) => {
    b.megacredits(2).slash().tag(Tag_1.Tag.SCIENCE).influence({ size: Size_1.Size.SMALL });
});
class SpinoffProducts extends GlobalEvent_1.GlobalEvent {
    ...
    resolve(game, turmoil) {
        game.getPlayersInGenerationOrder().forEach((player) => {
            player.stock.add(Resource_1.Resource.MEGACREDITS, 2 * (Math.min(5, player.tags.count(Tag_1.Tag.SCIENCE, 'raw-pf')) + turmoil.getPlayerInfluence(player)), { log: true, from: this.name });
        });
    }
}
