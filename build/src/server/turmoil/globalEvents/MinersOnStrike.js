...
const RENDER_DATA = CardRenderer_1.CardRenderer.builder((b) => {
    b.minus().titanium(1).slash().tag(Tag_1.Tag.JOVIAN).influence({ size: Size_1.Size.SMALL });
});
class MinersOnStrike extends GlobalEvent_1.GlobalEvent {
    ...
    resolve(game, turmoil) {
        game.getPlayersInGenerationOrder().forEach((player) => {
            ...
            if (amount > 0) {
                player.stock.deduct(Resource_1.Resource.TITANIUM, amount, { log: true, from: this.name });
            }
        });
    }
}
